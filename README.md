...
Four policies are published at `#/legal/privacy`, `#/legal/terms`,
`#/legal/cookies`, and `#/legal/refunds`, linked from the footer and the
contact card. The copy lives in `src/data/legal.ts`.

**These pages are not a shield.** They describe how this site actually
behaves, which is the thing that makes them defensible. They have not been
reviewed by an Indian advocate. Before using the site commercially, get them
reviewed.

### Does this site need a cookie consent banner?

**No**, and the reason is worth keeping in mind before adding anything.

Consent obligations — EU Article 5(3) of the ePrivacy Directive, and the
comparable position under India's DPDP Act — attach when a site *stores* data
on your device or *reads* data already there: cookies, `localStorage`,
`sessionStorage`, IndexedDB, or the Cache API.

This site does none of that. It sets no cookies, runs no analytics, and loads
nothing from a third-party origin, so there is nothing to consent to. A consent
banner here would be an empty checkbox, and shipping one is arguably worse than
shipping none because it implies tracking that does not happen.

If analytics, a newsletter, or a tracking pixel is ever added, that changes:

1. Write the privacy policy first, naming the processor and the data.
2. Add real consent, defaulting to **rejected**, before any storage is set.
3. Honour the choice on every subsequent visit.

### Data collected

None. There is no form, no API call, and no third-party embed. The contact
buttons are `mailto:` and `tel:` links, so correspondence travels through the
visitor's own mail or phone app rather than this site.

GitHub, as host, keeps its own server logs. That processing is GitHub's, under
its policy, and outside this project's control.

### DPDP Act 2023

The Act's duties attach to a Data Fiduciary that processes personal data of
users in India. This site processes none, so notice, consent, and the Data
Fiduciary obligations do not currently arise. The grievance contact and a
data-request route are published anyway, and a data request can be emailed to
`profile.email`.

Two things to know if this ever changes: once personal data is processed, a
Data Protection Officer may be required depending on volume and the nature of
the processing, and a grievance officer must be designated. Both are cheap to
add before they are compulsory.

### Third-party embeds and tracking

There are none, deliberately. Self-hosting the fonts (see `fetch_fonts.mjs`)
was a privacy decision as much as a performance one: a Google Fonts stylesheet
is a third party that can observe and alter every visitor's request. The
verified request count to any non-first-party origin is **0**.

Two plain links leave the site, the LinkedIn profile and a Google Maps location
search. Nothing is loaded from either until the visitor clicks.

### Third-party licences

The **Kanit** typeface is by The Kanit Project Authors under the **SIL Open
Font License 1.1**. Self-hosting a font is redistribution, which OFL requires
be accompanied by the license text, so `src/assets/fonts/OFL.txt` ships in the
repository. Do not delete it when trimming the font set.

The profile photograph is the site owner's own image, cropped and resized by
`scripts/make_profile_photo.py`. The favicon and icons are generated by
`scripts/build_favicon.mjs`.

### Claims on the page

Figures describing past work are presented as scope, not as guarantees or
service levels. Two self-awarded claims were removed:

- ~~"100% Service-First Approach"~~ — a score nothing measured, which
  implied a claim about other people. Replaced with a fact about how the work
  is done.
- The "99.9% uptime" figure is scoped to a specific deployment rather than
  presented as a general availability claim, because no continuous monitoring
  exists to support the latter.

### Verification

```bash
npm run check:legal      # policies render, are reachable, unknown slugs 404
npm run check:contrast   # WCAG 2.1 ratios for the palette actually used
npm run check:a11y       # tab order, focus rings, alt text, accessible names
npm run check:nav        # every nav link navigates
```

`check:contrast` alpha-composites the opacity modifiers over the real canvas
before measuring, so a value like `text-[#D7E2EA]/62` is reported at its
composited colour rather than the raw hex, which would be misleading.
...
├─ check-legal.mjs         policy pages render and unknown slugs 404
├─ check-contrast.mjs      WCAG ratios for the palette in use
├─ check-a11y.mjs          tab order, focus rings, alt text, names
├─ check-mobile-nav.mjs    every nav link navigates, via the mobile sheet
